...
class MiniCourt():
    ...
    def draw_court(self,frame):
        ...
        # Draw net
        net_start_point = (self.drawing_key_points[0], 
                           int((self.drawing_key_points[1] + self.drawing_key_points[5]) / 2)
                           )
        net_end_point = (self.drawing_key_points[2], 
                         int((self.drawing_key_points[1] + self.drawing_key_points[5]) / 2)
                        )
        cv2.line(frame, 
                 net_start_point, 
    ...
    def get_height_of_mini_court(self):
        return self.drawing_rectangle_height - (self.padding_court * 2)
    ...
    def get_mini_court_coordinates(self,
        ...
        distance_from_keypoint_x_pixels, distance_from_keypoint_y_pixels = measure_xy_distance(object_position, closest_kp)
        ...
        closest_mini_coourt_keypoint = (self.drawing_key_points[closest_kp_idx * 2],
    ...
    def convert_bboxes_to_mini_court_coordinates_player(self, player_boxes, original_court_key_points):
    ...
    def convert_bboxes_to_mini_court_coordinates_ball(self, ball_boxes, original_court_key_points):
        
        # source pts from original court
        ...
        # destination pts from mini court
        dst_kps = np.float32([
        ...
        H, _ = cv2.findHomography(src_pts, dst_kps)
        ...
        for frame_num, ball_bbox in enumerate(ball_boxes):
            ...
            ball_center = np.float32(get_center_of_bbox(ball_box))

            wrapped_pt = cv.perspectiveTransform(ball_center.reshape(-1, 1, 2), H)
            
            offset = 60
            # add the offset to the y coordinate
            wrapped_pt[0][0][1] += offset

            output_ball_boxes.append({1: (wrapped_pt[0][0][0], wrapped_pt[0][0][1])})
